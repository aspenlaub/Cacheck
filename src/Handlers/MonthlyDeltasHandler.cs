...
namespace Aspenlaub.Net.GitHub.CSharp.Cacheck.Handlers {
    public class MonthlyDeltasHandler : ISimpleCollectionViewSourceHandler {
        ...
        public MonthlyDeltasHandler(ICacheckApplicationModel model, IGuiAndAppHandler guiAndAppHandler) {
            vModel = model;
            vGuiAndAppHandler = guiAndAppHandler;
        }

        public async Task CollectionChangedAsync(IList<ICollectionViewSourceEntity> items) {
            vModel.MonthlyDeltas.Items.Clear();
            foreach (var item in items.Where(item => item.GetType() == vModel.MonthlyDeltas.EntityType)) {
                vModel.MonthlyDeltas.Items.Add(item);
            }
